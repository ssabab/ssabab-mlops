...
from pyspark.sql.functions import avg, countDistinct, col, floor, when, lit
...
food_df = spark.read.jdbc(mysql_url, "ssabab_dw.dim_food", properties=mysql_props)
...
user_stats_df = joined_df.groupBy("user_id") \
    .agg(
        avg("food_score").alias("user_avg_score"),
        countDistinct("category_id").alias("user_diversity_score")
    )

overall_stats = joined_df.groupBy().agg(
    avg("food_score").alias("group_avg_score"),
    countDistinct("category_id").alias("group_diversity_score")
...
dm_all = user_stats_df.crossJoin(overall_stats.select("group_type", "group_avg_score", "group_diversity_score")) \
    .select("user_id", "group_type", "user_avg_score", "group_avg_score", "user_diversity_score", "group_diversity_score")

dm_gender = user_stats_df.join(user_info, "user_id") \
    .join(gender_stats, (col("gender") == col("group_value")) & (col("group_type") == lit("gender"))) \
    .select("user_id", "group_type", "user_avg_score", "group_avg_score", "user_diversity_score", "group_diversity_score")

dm_age = user_stats_df.join(user_info, "user_id") \
    .join(age_stats, (col("age_group") == col("group_value")) & (col("group_type") == lit("age"))) \
    .select("user_id", "group_type", "user_avg_score", "group_avg_score", "user_diversity_score", "group_diversity_score")
