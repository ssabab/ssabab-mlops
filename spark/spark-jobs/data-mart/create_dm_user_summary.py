...
spark = SparkSession.builder \
    .appName("create_dm_user_summary") \
    .getOrCreate()
...
fact_user_pre_votes_df = spark.read.jdbc(
    url=mysql_url,
    table="ssabab_dw.fact_user_pre_votes",
    properties=mysql_properties
)

rating_summary_df = fact_user_ratings_df.groupBy("user_id") \
    .agg(
        avg("food_score").alias("avg_score"),
        count("food_score").alias("total_reviews")
    )

pre_vote_summary_df = fact_user_pre_votes_df.groupBy("user_id") \
    .agg(
